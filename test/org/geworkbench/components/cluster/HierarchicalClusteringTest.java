...
public class HierarchicalClusteringTest extends TestCase {
    ...
    double[][] dataD ={
    	{0.224325, 0.468666, 1.210881, 1.272600, 1.415378, 1.620722, 1.768653, 1.882424, 2.070221, 2.153244},
    	{0.244539, 0.722503, 1.201393, 1.311953, 1.680502, 1.858208, 2.172326, 2.274847, 2.256668, 2.271316},
    	{0.991929, 1.899524, 3.005665, 3.966271, 5.115155, 5.819132, 6.851253, 7.825265, 9.051731, 10.083811},
    	{1.136540, 2.044084, 2.943068, 3.865459, 5.200454, 5.929232, 7.116227, 8.125636, 9.115188, 10.057163},
    	{-0.148299, -0.889854, -1.229082, -1.293516, -1.446460, -1.699588, -2.194255, -1.924763, -2.411487, -2.060893},
    	{-0.175715, -0.683071, -1.113480, -1.414121, -1.784951, -1.646919, -2.185205, -1.998490, -2.334096, -2.177336},
    	{-0.799574, -1.917727, -2.941115, -4.236243, -4.923679, -5.931000, -6.972086, -7.763728, -9.059879, -9.750474},
    	{-1.217564, -1.901376, -3.186357, -4.123071, -4.895174, -5.750588, -6.945757, -7.948440, -8.956685, -9.857914}
    	};
     ...
     * @throws Exception
     */
    public void testLinkageClustering() throws Exception {
...
//        ClusterConverter helper = new ClusterConverter(factory);
        //Single linkage
        SimpleClustering slinkage = new SimpleClustering(HClustering.Linkage.SINGLE);
        HierCluster clusterE = slinkage.compute(null, data, factory, EuclideanDistance.instance);
        ...
        //Complete linkage
        SimpleClustering clinkage = new SimpleClustering(HClustering.Linkage.COMPLETE);
        HierCluster cclusterE = clinkage.compute(null, data, factory, EuclideanDistance.instance);
        HierCluster cclusterS = clinkage.compute(null, data, factory, SpearmanRankDistance.instance);
        HierCluster cclusterC = clinkage.compute(null, data, factory, CorrelationDistance.instance);
        //Average linkage
        SimpleClustering alinkage = new SimpleClustering(HClustering.Linkage.AVERAGE);
        HierCluster aclusterE = alinkage.compute(null, data, factory, EuclideanDistance.instance);
        HierCluster aclusterS = alinkage.compute(null, data, factory, SpearmanRankDistance.instance);
        HierCluster aclusterC = alinkage.compute(null, data, factory, CorrelationDistance.instance);
     ...
     * @throws Exception
     */
    public void testDistanceClustering() throws Exception {
    	// Testdata see "Hierarchical Clustering.doc" (in the GUI tests) for more information and how it looks like
        
        
        HierClusterFactory.Test factory = new HierClusterFactory.Test(itemsD);
//        ClusterConverter helper = new ClusterConverter(factory);
        //Single linkage
        SimpleClustering slinkage = new SimpleClustering(HClustering.Linkage.SINGLE);
        HierCluster clusterE = slinkage.compute(null, dataD, factory, EuclideanDistance.instance);
        ...
        //Complete linkage
        SimpleClustering clinkage = new SimpleClustering(HClustering.Linkage.COMPLETE);
        HierCluster cclusterE = clinkage.compute(null, dataD, factory, EuclideanDistance.instance);
        HierCluster cclusterS = clinkage.compute(null, dataD, factory, SpearmanRankDistance.instance);
        HierCluster cclusterC = clinkage.compute(null, dataD, factory, CorrelationDistance.instance);
        //Average linkage
        SimpleClustering alinkage = new SimpleClustering(HClustering.Linkage.AVERAGE);
        HierCluster aclusterE = alinkage.compute(null, dataD, factory, EuclideanDistance.instance);
        HierCluster aclusterS = alinkage.compute(null, dataD, factory, SpearmanRankDistance.instance);
        HierCluster aclusterC = alinkage.compute(null, dataD, factory, CorrelationDistance.instance);
        ...
        //
        assertTrue("D1",  ClusterUtils.areClustersEqual(hcE[0], clusterEO));
        assertFalse("D2", ClusterUtils.areClustersEqual(hcE[0], clusterSO));
        assertFalse("D3", ClusterUtils.areClustersEqual(hcE[0], clusterCO));
        
        assertFalse("D4", ClusterUtils.areClustersEqual(hcE[0], cclusterE));
        assertFalse("D5", ClusterUtils.areClustersEqual(hcE[0], cclusterS));
        assertFalse("D6", ClusterUtils.areClustersEqual(hcE[0], cclusterC));
               
        assertFalse("D7", ClusterUtils.areClustersEqual(hcE[0], aclusterE));
        assertFalse("D8", ClusterUtils.areClustersEqual(hcE[0], aclusterS));
        assertFalse("D9", ClusterUtils.areClustersEqual(hcE[0], aclusterC));
        
        //
        assertFalse("D10", ClusterUtils.areClustersEqual(hcE2[0], cclusterE));
        assertFalse("D11", ClusterUtils.areClustersEqual(hcE2[0], cclusterS));
        assertTrue("D12", ClusterUtils.areClustersEqual(hcE2[0], cclusterC));
        
        assertFalse("D13", ClusterUtils.areClustersEqual(hcE2[0], clusterEO));
        assertFalse("D14", ClusterUtils.areClustersEqual(hcE2[0], clusterSO));
        assertTrue("D15", ClusterUtils.areClustersEqual(hcE2[0], clusterCO));
        
        assertFalse("D16", ClusterUtils.areClustersEqual(hcE2[0], aclusterE));
        assertFalse("D17", ClusterUtils.areClustersEqual(hcE2[0], aclusterS));
        assertTrue("D18", ClusterUtils.areClustersEqual(hcE2[0], aclusterC));
...
*/
    ...
    public static void main(String[] args) {
       HierarchicalClusteringTest t = new HierarchicalClusteringTest("web100.exp");
        try {
        	t.testLinkageClustering();
        	t.testDistanceClustering();
        }
        catch (Exception e){};          
