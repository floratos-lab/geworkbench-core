		...
		if (h.isLeaf()) {
			...
			if (h instanceof MarkerHierCluster) {
				System.out.println("" + h.getHeight() + " ("
						+ ((MarkerHierCluster) h).getMarkerInfo().getLabel()
						+ ")");
	...
	public static boolean areClustersEqual(HierCluster h1, HierCluster h2) {
		if (h1.getDepth() == h2.getDepth()) {
			if (h1.isLeaf() && h2.isLeaf()) {
				if ((h1 instanceof MarkerHierCluster)
						&& (h2 instanceof MarkerHierCluster)) {
					if (h1.getHeight() == h2.getHeight()) {
						DSGeneMarker marker1 = ((MarkerHierCluster) h1)
								.getMarkerInfo();
						DSGeneMarker marker2 = ((MarkerHierCluster) h2)
								.getMarkerInfo();
				...
				} else if ((h1 instanceof MicroarrayHierCluster)
						&& (h2 instanceof MicroarrayHierCluster)) {
					if (h1.getHeight() == h2.getHeight()) {
						DSMicroarray microarray1 = ((MicroarrayHierCluster) h1)
								.getMicroarray();
						DSMicroarray microarray2 = ((MicroarrayHierCluster) h2)
								.getMicroarray();
						if (microarray1.getLabel().equals(
								microarray2.getLabel())) {
							return true;
						} else {
							return false;
						}
				...
				if (areClustersEqual(h1.getNode(0), h2.getNode(0))
						&& areClustersEqual(h1.getNode(1), h2.getNode(1))) {
					return true;
				} else if (areClustersEqual(h1.getNode(1), h2.getNode(0))
						&& areClustersEqual(h1.getNode(0), h2.getNode(1))) {
					return true;
				} else {
					return false;
				}
