...
public class AdjacencyMatrix extends BWAbstractAlgorithm implements
	...
	public void printGene(DSMicroarraySet<DSMicroarray> mArraySet,
			File writeFile, Map map, double miThresh, int markerIndex) {
		try {
			...
			while (miSetIt.hasNext()) {
				...
				double miVal = Double
						.parseDouble(miEntry.getValue().toString());
				// if (miVal >= 0.0) {
				if (miVal > miThresh) {
	...
	public void print(DSMicroarraySet<DSMicroarray> mArraySet, File writeFile,
			Map map, double miThresh, int startIndex, int endIndex) {
		try {
			FileWriter writer = new FileWriter(writeFile);
			...
			for (int geneRowCtr = startIndex; geneRowCtr < endIndex; geneRowCtr++) {
				...
				// int markerIndex =
				// Integer.parseInt(curEntry.getKey().toString());
	...
	public void print(DSMicroarraySet<DSMicroarray> mArraySet, File writeFile,
		...
		try {
			...
			while (entrySetIt.hasNext()) {
				Map.Entry curEntry = (Map.Entry) entrySetIt.next();
				int markerIndex = Integer
						.parseInt(curEntry.getKey().toString());
				// String markerName = mArraySet.getMarkerLabel(markerIndex);
	...
	public void addDirectional(int geneId1, int geneId2, float edge) {
		...
		if ((geneId1 >= 0) && (geneId2 >= 0)) {
			...
			HashMap<Integer, Float> row = (HashMap<Integer, Float>) geneRows
	...
	public void addDirectional(int geneId1, int geneId2, String interaction) {
		...
		/*
		 * int bin = Math.min(1023, (int) (edge * edgeScale)); if (bin >= 0) {
		 * histogram[bin]++; }
		 */
	...
	public void execute() {
		if (parms.dataMatrixName != null) {
			...
			if (miLib.initialized()) {
				miLib.setParams(parms.mean, parms.variance, parms.miThreshold,
						parms.miErrorPercent, parms.reduce);
				if (parms.controlId < 0) {
					miLib.create(parms.dataMatrixName, parms.adjMatrixName,
							parms.mArrayNo, parms.mArrayId);
				} else {
					miLib.createConstrainedLow(parms.dataMatrixName,
							parms.controlId, parms.percent);
					miLib.createConstrainedHigh(parms.dataMatrixName,
							parms.controlId, parms.percent);
				}
			}
		}
	...
	public String getMarkerNameGW(int index) {
		if (keyMapping.get(new Integer(index)) == null) {
			/*
			 * String tmp = maSet.getGenericMarker(index).toString(); int s1 =
			 * tmp.indexOf("(") + 1; int s2 = tmp.indexOf(")"); String genename =
			 * tmp.substring(s1, s2); // System.out.println("addMarkerName "+
			 * description); return genename;
			 */
	...
	public void read(String name, float miThresh) {
		...
		try {
			...
			try {
					...
					if (line.length() > 0 && line.charAt(0) != '-') {
						...
						if (geneId1 >= 0) {
							while (tr.hasMoreTokens()) {
								...
								int geneId2 = getMappedId(Integer
										.parseInt(strGeneId2));
								if (geneId2 >= 0) {
									...
									if (mi > miThresh) {
										if (geneId1 != geneId2) {
											...
											// this.addInteractionType2(geneId1,
											// geneId2, mi);
										} else {
											connectionsIgnored++;
										}
				...
				System.out.println("Connections instantiated "
						+ connectionsInstantiated);
	...
	public void read(String name, HashMap accessionsMap, Vector geneNames,
			float miThresh) {
		int connectionsInstantiated = 0;
		int connectionsIgnored = 0;
			...
			try {
				...
				while (br.ready()) {
					...
					if (line.length() > 0 && line.charAt(0) != '-') {
						...
						String geneName = (String) accessionsMap
								.get(geneAccess);
						...
						if (geneName != null && geneNames.contains(geneName)
							...
							if (geneId1 >= 0) {
								while (tr.hasMoreTokens()) {
									String strGeneId2 = new String(tr
											.nextToken());
									String strMi = new String(tr.nextToken());
									float mi = Float.parseFloat(strMi);
									int geneId2 = getMappedId(Integer
			...
			try {
				...
				while (br.ready()) {
					if (line.length() > 0 && line.charAt(0) != '-') {
						...
						String geneName = (String) keyMapping.get(new Integer(
								geneId1));
						geneId1 = -1;
						if (geneName != null) {
							DSGeneMarker m = microarraySet.getMarkers().get(
									geneName);
							if (m != null) {
								geneId1 = m.getSerial();
							} else {
						...
						if (geneId1 > -1) {
							...
							if (geneId1 >= 0) {
								while (tr.hasMoreTokens()) {
									...
									int geneId2 = Integer.parseInt(tr
											.nextToken());
									String geneName2 = (String) keyMapping
											.get(new Integer(geneId2));
									if (geneName2 == null) {
										geneName2 = microarraySet.getMarkers()
												.get(geneId2).getLabel();
									}
									geneId2 = -1;
									if (geneName2 != null) {
										DSGeneMarker m = microarraySet
												.getMarkers().get(geneName2);
										if (m != null) {
											geneId2 = m.getSerial();
										} else {
											tr.nextToken();
	...
	public double computeBayes(DSMicroarraySet microarraySet,
			DSItemList<DSMicroarray> mArrays, double bdePrior,
			boolean directional, String searchAlg) {
		maSet = microarraySet;
		BayesUtil util = new BayesUtil();
		Table rawData = util.convertToTable(microarraySet, mArrays);
		return computeBayes(microarraySet, rawData, bdePrior, directional,
				searchAlg);
	}

	public double computeBayes(DSMicroarraySet microarraySet, double bdePrior,
			String searchAlg) {
		maSet = microarraySet;
		BayesUtil util = new BayesUtil();
	...
	public double computeBayes(DSMicroarraySet microarraySet, Table rawData,
			double bdePrior, String searchAlg) {
		maSet = microarraySet;
		return computeBayes(microarraySet, rawData, bdePrior, false, searchAlg);
	}

	public double computeBayes(DSMicroarraySet<DSMicroarray> microarraySet,
			Table rawData, double bdePrior, boolean directional,
			String searchAlg) {
		maSet = microarraySet;
		...
		ScoreBasedLearner sl = BayesSLFactory.getLearner(searchAlg,
				discretizedData);
		...
		System.out.println("Computing Bayes Graph for bde " + bdePrior
				+ " With search Algorithm " + sl.getClass().getName());
		...
		while (edgeIter.hasNext()) {
			...
			if (m != null) {
				...
				if (m != null) {
					...
					if (directional) {
						this.addDirectional(sourceNodeIndex, destNodeIndex,
								0.05f);
					} else {
	...
	public void computeCopula(DSMicroarraySet microarraySet, double threshold,
			DSItemList<DSMicroarray> mArrays, double sigma) {
		maSet = microarraySet;
		if (microarraySet != null) {
			...
			GeneGeneRelationship ggr = new GeneGeneRelationship(view,
					GeneGeneRelationship.RANK_CHI2, false, sigma);
	...
	// We gotta separate all this stuff a lot better
	public void computeCopula(DSMicroarraySet microarraySet, double threshold,
			DSItemList<DSMicroarray> mArrays, double sigma, int i) {
		maSet = microarraySet;
		if (microarraySet != null) {
			int markerNo = microarraySet.size();
			DSMicroarraySetView<DSGeneMarker, DSMicroarray> view = new CSMicroarraySetView<DSGeneMarker, DSMicroarray>();
			...
			view.useItemPanel(false);
			view.useMarkerPanel(false);
			GeneGeneRelationship ggr = new GeneGeneRelationship(
					view,
					org.geworkbench.util.pathwaydecoder.GeneGeneRelationship.RANK_CHI2,
					false, sigma);
			ggr.setCopulaTransform(true);
	...
	public void compute(DSMicroarraySet microarraySet, double threshold,
			DSItemList<DSMicroarray> mArrays) {
		compute(
				microarraySet,
				threshold,
				mArrays,
				org.geworkbench.util.pathwaydecoder.GeneGeneRelationship.GAUSSIAN_KERNEL);
	}

	public void compute(DSMicroarraySet microarraySet, double threshold,
			DSItemList<DSMicroarray> mArrays, int miCalcType) {
		maSet = microarraySet;
		if (microarraySet != null) {
			int markerNo = microarraySet.size();
			org.geworkbench.util.pathwaydecoder.GeneGeneRelationship ggr;
			if (miCalcType == GeneGeneRelationship.GAUSSIAN_KERNEL) {
				ggr = new GeneGeneRelationship(
						microarraySet,
						microarraySet,
						mArrays.size(),
						org.geworkbench.util.pathwaydecoder.GeneGeneRelationship.RANK_CHI2,
				...
				ggr = new GeneGeneRelationship(
						...
						microarraySet,
						mArrays.size(),
						org.geworkbench.util.pathwaydecoder.GeneGeneRelationship.RANK_MI,
						false);
			}
			for (int i = 0; i < markerNo; i++) {
				for (int j = i + 1; j < markerNo; j++) {
					...
					// double pvalue =
					// PathwayDecoderUtil.getPValue(edge.getMI(),
					// microarraySet.size());
	...
	public void compute(DSMicroarraySet microarraySet, double threshold,
		...
		if (microarraySet != null) {
			...
			if (miCalcType == GeneGeneRelationship.GAUSSIAN_KERNEL) {
				ggr = new GeneGeneRelationship(view,
						GeneGeneRelationship.RANK_CHI2, false);
			} else {
				ggr = new GeneGeneRelationship(view,
						GeneGeneRelationship.RANK_MI, false);
			}
	...
	public void clean(DSMicroarraySet microarraySet, double threshold,
			double eps, int startId, int endId) {
		int edgesRemoved = 0;
		int edgesRetained = 0;
		...
		// Now we remove edges that satisfy the data processing inequality with
		// tolerance eps
	...
	public void clean_new(DSMicroarraySet microarraySet, double threshold,
			double eps) {
		if (microarraySet != null) {
			maSet = microarraySet;
		}
	...
	/**
	 * the clean() function called from GeneNetworkPanel -> CreateNetwork() this
	 * function is GW compatible
	 * 
	 * @param microarraySet
	 *            DSMicroarraySet
	 * @param threshold
		...
		for (Iterator geneA = rowIds.iterator(); geneA.hasNext();) {
			...
			for (Iterator geneB = colIds.iterator(); geneB.hasNext();) {
				...
				if (miAB < 0) {
					...
					// added a new method to change the MI strength for GW
					// interaction
					// System.out.println("Negative miAB found: " + geneAId + "
					// + " + geneBId);
					...
					// then we register the same change in the GeneWays data
					// structure
					// subclass -> this.changeInteractionType2Strength(geneAId,
					// geneBId, -miAB);
					// this.add(geneAId, geneBId, -1.0f);
				}
				if (Math.abs(miAB) < threshold) {
					...
					// hey, this line is necessary for Geneways !!
					// subclassed ->
					// this.changeInteractionType2Strength(geneAId, geneBId,
					// -Math.abs(miAB));
					// this.add(geneAId, geneBId, -1.0f);
				}
			}
	...
	/**
	 * This method parses the genes and adds them into a look up table where
	 * identical genes are resolved
	 */
	...
	public void resolveGeneCollision(DSMicroarraySet microarraySet) {
		...
		/*
		 * if(maSet != null) { for (int i = 0; i < maSet.size(); i++) {
		 * 
		 * IGenericMarker gm = maSet.getGenericMarker(i); int locusLink =
		 * gm.getLocusLink(); if (locusLink != 0) { ArrayList gmList =
		 * (ArrayList) locusLinkMap.get(new Integer(locusLink)); if (gmList !=
		 * null) { if (!gmList.contains(gm)) { gmList.add(gm); } } else { gmList =
		 * new ArrayList(); gmList.add(gm); locusLinkMap.put(new
		 * Integer(locusLink), gmList); } } } }
		 */
	}
	...
	private int getUniID(DSGeneMarker marker) {
		...
		if (id == -1) {
			// Ensure a unique, but degenerate, unigene ID is returned if none
			// is found
			id = --emptyUniID;
		}
		return id;
	 ...
	 *            IGenericMarker
	 */
	public void cleanFirstNeighbors(DSMicroarraySet microarraySet,
			DSGeneMarker gm0) {
		if (microarraySet != null) {
			maSet = microarraySet;
		}
		...
		for (Iterator geneA = rowIds.iterator(); geneA.hasNext();) {
			Integer geneAKey = (Integer) geneA.next();
			int geneAId = geneAKey.intValue();
			int uniId1 = maSet.getMarkers().get(geneAId).getUnigene()
					.getUnigeneId();
			if (uniId1 == -1) {
		...
		for (Iterator geneA = rowIds.iterator(); geneA.hasNext();) {
			...
			for (Iterator geneB = colIds.iterator(); geneB.hasNext();) {
				...
				float mi = this.get(geneAId, geneBId);
				// remove the edge between geneA and geneB
				log.debug("Setting interaction between " + geneAId + " and "
						+ geneBId + " to " + (-Math.abs(mi)));
		...
		while (!allGenes.isEmpty() && decrease) {
			// find the best connection to any of the genes in the current
			// selection.
			decrease = false;
			...
			for (Iterator geneA = rowIds.iterator(); geneA.hasNext();) {
				...
				if (allGenes.contains(geneAKey)) {
					...
					if (!completed.contains(new Integer(unigeneId))) {
						...
						int maxId = -1;
						Set colIds = geneRow.keySet();
						for (Iterator geneB = colIds.iterator(); geneB
								.hasNext();) {
							Integer geneBKey = (Integer) geneB.next();
							int geneBId = geneBKey.intValue();
						...
						// what is maxID?????
						if (maxId != -1) {
							// Now remove geneA from allgenes and add it to the
							// new neighbors
							...
							// GW compatible
							// subclass ->
							// this.changeInteractionType2Strength(geneAId,
							// maxId, maxMI);
	...
	public void printGeneNamesAndAccessions(
			DSMicroarraySet<DSMicroarray> mArraySet, File writeFile) {
		...
		try {
			...
			while (geneRowsIt.hasNext()) {
				...
				// geneRowKey).
				// split(",");
				// writer.write(geneRowKey + ":" + geneRowAccession + ":" +
				// arrGeneRowName[0]);

				// String geneName = (String) geneNameMap.get(geneRowAccession);
				String geneName = mArraySet.getMarkers().get(geneRowKey)
						.getDescription();
				writer.write(geneRowKey + ":" + geneRowAccession + ":"
						+ geneName);
				...
				while (targetsIterator.hasNext()) {
					...
					float targetMi = ((Float) targetEntry.getValue())
							.floatValue();
					...
					writer.write("\t" + targetId + ":" + targetAccession + ":"
							+ targetName + ":" + strMi);
				}
				writer.write("\n");
			}
	...
	public void printTargetsOfGene(DSMicroarraySet<DSMicroarray> mArraySet,
			File writeFile, String centerAccession) {
		try {
			DSGeneMarker m = mArraySet.getMarkers().get(centerAccession);
			if (m != null) {
				FileWriter writer = new FileWriter(writeFile);
				int markerIndex = m.getSerial();
				// HashMap geneRow = (HashMap) geneRows.get(new
				// Integer(centerAccession));
				HashMap geneRow = (HashMap) geneRows.get(new Integer(
						markerIndex));
				if (geneRow == null) {
				...
				for (Integer key2 : row.keySet()) {
					...
					if (sn2 != null && !sn2.equals("---")) {
						if (!adjMat.get(sn.trim()).containsKey(sn2.trim())) {
							adjMat.get(sn.trim())
									.put(sn2.trim(), row.get(key2));
						} else {
							float prevMI = adjMat.get(sn.trim())
									.get(sn2.trim());
							if (prevMI < row.get(key2)) {
								adjMat.get(sn.trim()).put(sn2.trim(),
										row.get(key2));
							}
						}
					}
		...
		try {
			...
			for (String key : adjMat.keySet()) {
				...
				for (String key2 : adjMat.get(key).keySet()) {
					bw.write("\t" + key2 + "\t"
							+ adjMat.get(key).get(key2).toString().trim());
				}
				bw.write("\n");
			}
			bw.flush();
