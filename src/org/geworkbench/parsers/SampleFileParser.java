		...
		try {
			...
			if (in != null) {
				...
				while ((header = in.readLine()) != null) {
					/*
					 * Adding comments to Experiment Information tab.We will
					 * ignore the line which start with '!platform_table_end',
					 * '!platform_table_begin', '!sample_table_begin'and
					 * '!sample_table_end'
					 */
					if (header.startsWith(commentSign1)
							|| header.startsWith(commentSign2)) {
						if (!header.equalsIgnoreCase("!sample_table_end")
								&& !header
										.equalsIgnoreCase("!sample_table_begin")) {
							// to be consistent, this detailed information
							// should be used else where instead of as
							// "description" field
					...
					if (header.startsWith(commentSign2)) {
						...
						if (temP1.equals("!Sample_title")) {
							String temp1 = tempName + ": " + temp[1].trim();
							arrayName = temp1;
						}
					}
					if (!header.startsWith(commentSign1)
							&& !header.startsWith(commentSign2)
							&& !header.startsWith(commentSign3)) {
						...
						if (!header.subSequence(0, 6).equals("ID_REF")) {
							...
							CSExpressionMarker marker = new CSExpressionMarker(
									m);
						...
						if (valString == null) {
							...
							if (detection != 0 || call != 0) {
								if (detection != 0) {
									...
									markerValue.setConfidence(((Double) value)
											.doubleValue());
									pValueFound = true;
								}
								if (call != 0) {
							...
							if (detection != 0 || call != 0) {
								if (detection != 0) {
									...
									markerValue.setConfidence(((Double) value1)
											.doubleValue());
									pValueFound = true;
								}
								if (call != 0) {
