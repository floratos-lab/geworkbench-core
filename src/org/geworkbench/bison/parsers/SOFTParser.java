...
public class SOFTParser {
    ...
    public void process(String line) {
        ...
        if(line.startsWith("!"))
        	return;
        if (!line.substring(0, 6).contentEquals("ID_REF")) {
            if (!headerFound)
                return;
        } else {
            headerFound = true;
    ...
    public void parseLine(String line) {
        ...
        if (st.hasMoreTokens()) {
            token = st.nextToken().trim();
            int tokenIndex = 1;
            SOFTParseContext context = new org.geworkbench.bison.parsers.SOFTParseContext(columnsToUse);
            ...
            ( (CSAffyMarkerValue) microarray.getMarkerValue(markerIndex++)).initS(context);
