...
public class IdeaEdge implements Serializable, Comparable<IdeaEdge> {
	//Edge is the gene pair which is expanded on probe Ids. For each edge, the geneNo1 and geneNo2 which is entrez gene number
	//may be the same, but probe Ids which is expRowNoG1 and expRowNoG2 are unique. 
	...
	public enum InteractionType {PROTEIN_PROTEIN, PROTEIN_DNA};
	...
	public IdeaEdge(int geneNo1,int geneNo2,DSGeneMarker marker1,DSGeneMarker marker2, int expRowNoG1, int expRowNoG2, String probeId1, String probeId2, InteractionType ppi){
	...
	public int compareTo(IdeaEdge otherEdge) {
		if ((this.expRowNoG1==otherEdge.getExpRowNoG1())&&(this.expRowNoG2==otherEdge.getExpRowNoG2())) 
			return 0;
		else if((this.expRowNoG1+this.expRowNoG2)>(otherEdge.getExpRowNoG1()+otherEdge.getExpRowNoG2())) return 1;
		else return -1;
	}
