...
public class ModelBuilderCommandLanguage implements IModelBuilder, IAliasableClass {
    ...
    private void handleStatementSymbolDefinition(ListIterator pTokenIter, Model pModelHashMap, HashMap pSymbolMap) throws InvalidInputException, DataNotFoundException {
        ...
        if (null != foundSymbolValue) {
            ...
            if (null == foundSymbolValue.getValue() || foundSymbolValue.getClass().getSuperclass().equals(Object.class)) {
    ...
    private void getReactionParticipants(ListIterator pTokenIter, HashMap pSymbolMap, HashMap pSpeciesStoicMap, HashMap pSpeciesDynamicMap, Reaction.ParticipantType pParticipantType) throws InvalidInputException, DataNotFoundException {
        while (pTokenIter.hasNext()) {
            ...
            token = getNextToken(pTokenIter);
            if (pParticipantType.equals(Reaction.ParticipantType.REACTANT) && token.mCode.equals(Token.Code.HYPHEN)) {
                token = getNextToken(pTokenIter);
                assert (token.mCode.equals(Token.Code.GREATER_THAN)) : "expected greater-than symbol";
                break;
            } else if (token.mCode.equals(Token.Code.PLUS)) {
                continue;
            } else if (pParticipantType.equals(Reaction.ParticipantType.PRODUCT) && token.mCode.equals(Token.Code.COMMA)) {
                break;
            } else {
    ...
    private void handleSpeciesDefinitions(Reaction pReaction, Reaction.ParticipantType pReactionParticipantType, HashMap pSymbolMap, HashMap pSpeciesStoicMap, HashMap pSpeciesDynamicMap) throws InvalidInputException {
    ...
    private void handleStatementReaction(ListIterator pTokenIter, Model pModel, HashMap pSymbolMap, MutableInteger pNumReactions) throws InvalidInputException, DataNotFoundException {
        ...
        if (hasReactants) {
            getReactionParticipants(pTokenIter, pSymbolMap, speciesStoicMap, speciesDynamicMap, Reaction.ParticipantType.REACTANT);

            handleSpeciesDefinitions(reaction, Reaction.ParticipantType.REACTANT, pSymbolMap, speciesStoicMap, speciesDynamicMap);
        } else {
            pTokenIter.next();
            pTokenIter.next();
        }
        ...
        if (hasProducts) {
            getReactionParticipants(pTokenIter, pSymbolMap, speciesStoicMap, speciesDynamicMap, Reaction.ParticipantType.PRODUCT);

            handleSpeciesDefinitions(reaction, Reaction.ParticipantType.PRODUCT, pSymbolMap, speciesStoicMap, speciesDynamicMap);
        }
