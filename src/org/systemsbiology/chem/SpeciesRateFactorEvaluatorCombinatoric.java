...
public class SpeciesRateFactorEvaluatorCombinatoric extends SpeciesRateFactorEvaluator {
    ...
    public final double computeRateFactorForSpecies(SymbolEvaluator pSymbolEvaluator, Species pSpecies, int pStoichiometry) throws DataNotFoundException {
        double speciesValue = pSymbolEvaluator.getValue(pSpecies.getSymbol());
        double numReactantCombinations = 1.0;

        if (pStoichiometry > 1) {
            if (speciesValue < MIN_POPULATION_FOR_COMBINATORIC_EFFECTS && speciesValue - Math.floor(speciesValue) == 0.0) {
