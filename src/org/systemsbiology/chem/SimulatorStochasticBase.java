...
public abstract class SimulatorStochasticBase extends Simulator {
    ...
    protected static final void updateSymbolValuesForReaction(SymbolEvaluatorChemSimulation pSymbolEvaluator, Reaction pReaction, double[] pSymbolValues, DelayedReactionSolver[] pDelayedReactionSolvers) throws DataNotFoundException {
        int numDelayedReactions = pDelayedReactionSolvers.length;
    ...
    protected static final double chooseDeltaTimeToNextReaction(Random pRandomNumberGenerator, double pReactionProbability) {
    ...
    protected abstract double iterate(SpeciesRateFactorEvaluator pSpeciesRateFactorEvaluator, SymbolEvaluatorChemSimulation pSymbolEvaluator, double pEndTime, Reaction[] pReactions, double[] pReactionProbabilities, Random pRandomNumberGenerator, double[] pDynamicSymbolValues, MutableInteger pLastReactionIndex, DelayedReactionSolver[] pDelayedReactionSolvers) throws DataNotFoundException, IllegalStateException;

    protected abstract void prepareForStochasticSimulation(SpeciesRateFactorEvaluator pSpeciesRateFactorEvaluator, SymbolEvaluatorChemSimulation pSymbolEvaluator, double pStartTime, Random pRandomNumberGenerator, Reaction[] pReactions, double[] pReactionProbabilities) throws DataNotFoundException;

    public final void simulate(double pStartTime, double pEndTime, SimulatorParameters pSimulatorParameters, int pNumResultsTimePoints, String[] pRequestedSymbolNames, double[] pRetTimeValues, Object[] pRetSymbolValues) throws DataNotFoundException, IllegalStateException, IllegalArgumentException {
        ...
        for (long simCtr = ensembleSize; --simCtr >= 0;) {
            ...
            prepareForStochasticSimulation(speciesRateFactorEvaluator, symbolEvaluator, pStartTime, randomNumberGenerator, reactions, reactionProbabilities);

            while (pNumResultsTimePoints - timePointIndex > 0) {
                time = iterate(speciesRateFactorEvaluator, symbolEvaluator, pEndTime, reactions, reactionProbabilities, randomNumberGenerator, dynamicSymbolValues, lastReactionIndex, delayedReactionSolvers);
                ...
                if (time >= timesArray[timePointIndex]) {
                    ...
                    timePointIndex = addRequestedSymbolValues(time, timePointIndex, requestedSymbols, symbolEvaluator, timesArray, pRetSymbolValues);
                }
            }
