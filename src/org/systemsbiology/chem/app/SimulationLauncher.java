...
import org.systemsbiology.util.ClassRegistry;
import org.systemsbiology.util.FramePlacer;
...
public class SimulationLauncher {
    ...
    private org.systemsbiology.util.Queue mResultsQueue;
     ...
     */
    public SimulationLauncher(String pAppName, Model pModel, boolean pHandleOutputInternally) throws ClassNotFoundException, IOException, InstantiationException {
        mAppName = pAppName;
        ...
        if (mHandleOutputInternally) {
            ...
            mResultsQueue = new org.systemsbiology.util.Queue();
            mPlotter = null;
        }
        mOutputFile = null;
    ...
    private void showCancelledSimulationDialog() {
        SimpleDialog messageDialog = new SimpleDialog(getLauncherFrame(), "Simulation cancelled", "Your simulation has been cancelled");
    ...
    private void handleOutput(OutputType pOutputType, String pOutputPlotLabel, String pOutputFileName, TimeSeriesOutputFormat pOutputFileFormat, boolean pOutputFileAppend, String[] pRequestedSymbolNames, double[] pTimeValues, Object[] pSymbolValues) throws IOException {
        ...
        TimeSeriesSymbolValuesReporter.reportTimeSeriesSymbolValues(printWriter, pRequestedSymbolNames, pTimeValues, pSymbolValues, pOutputFileFormat);

        if (pOutputType.equals(OutputType.PRINT)) {
            JTextArea outputTextArea = new JTextArea(OUTPUT_TEXT_AREA_NUM_ROWS, OUTPUT_TEXT_AREA_NUM_COLS);
    ...
    private void runSimulation(SimulationRunParameters pSimulationRunParameters) {
        ...
        try {
            ...
            simulator.simulate(pSimulationRunParameters.mStartTime, pSimulationRunParameters.mEndTime, pSimulationRunParameters.mSimulatorParameters, pSimulationRunParameters.mNumTimePoints, pSimulationRunParameters.mRequestedSymbolNames, pSimulationRunParameters.mRetTimeValues, pSimulationRunParameters.mRetSymbolValues);
            ...
            if (!mSimulationController.getCancelled()) {
                if (mHandleOutputInternally) {
                    ...
                    handleOutput(pSimulationRunParameters.mOutputType, pSimulationRunParameters.mOutputPlotLabel, pSimulationRunParameters.mOutputFileName, pSimulationRunParameters.mOutputFileFormat, pSimulationRunParameters.mOutputFileAppend, pSimulationRunParameters.mRequestedSymbolNames, pSimulationRunParameters.mRetTimeValues, pSimulationRunParameters.mRetSymbolValues);
                } else {
            ...
            ExceptionDialogOperationCancelled dialog = new ExceptionDialogOperationCancelled(getLauncherFrame(), "Failure running simulation", e);
            dialog.show();
        } catch (Throwable e) {
            simulationEndCleanup(simulator);
            e.printStackTrace(System.err);
            ExceptionDialogOperationCancelled dialog = new ExceptionDialogOperationCancelled(getLauncherFrame(), "Failure running simulation", new Exception(e.toString()));
            dialog.show();
    ...
    private void handleSimulatorSelection(int pSimulatorIndex) {
        ...
        if (null == simulatorAlias) {
            ...
            try {
                ...
                ExceptionDialogOperationCancelled dialog = new ExceptionDialogOperationCancelled(getLauncherFrame(), "Failed to instantiate simulator", e);
                dialog.show();
            }
        }
    }
    ...
    private void handleBadInput(String pTitle, String pMessage) {
        SimpleDialog dialog = new SimpleDialog(getLauncherFrame(), pTitle, pMessage);
        dialog.show();
    }
    ...
    private void handleOutputFileMouseClick() {
        ...
        if (outputType.equals(OutputType.FILE)) {
            ...
            if (null != outputFile) {
                ...
                if (outputFile.exists() && (null == mOutputFile || !mOutputFile.equals(outputFile)) && !mOutputFileAppendCheckBox.isSelected()) {
        ...
        outputFileAppendCheckBox.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                if (!append && null != mOutputFile && mOutputFile.exists()) {
                    boolean proceed = FileChooser.handleOutputFileAlreadyExists(mLauncherFrame, mOutputFile.getAbsolutePath());
                    if (!proceed) {
