...
public class EditorPane {
    ...
    private void handleCancel(String pOperation) {
        SimpleDialog messageDialog = new SimpleDialog(mMainFrame, pOperation + " cancelled", "Your " + pOperation + " operation has been cancelled");
    ...
    public void handleResize(int changeWidthPixels, int changeHeightPixels) {
        ...
        if (widthPixels > 0 && heightPixels > 0) {
            int widthChars = (int) Math.floor(((double) widthPixels) / ((double) originalWidthPixels) * ((double) EDITOR_TEXT_AREA_NUM_COLS));

            int heightChars = (int) Math.floor(((double) heightPixels) / ((double) originalHeightPixels) * ((double) EDITOR_TEXT_AREA_NUM_ROWS));
            ...
            mEditorScrollPane.setPreferredSize(new Dimension(widthPixels + SCROLL_PANE_OFFSET_PIXELS, heightPixels + SCROLL_PANE_OFFSET_PIXELS));
    ...
    public boolean close() {
        ...
        if (getBufferDirty()) {
            ...
            SimpleDialog messageDialog = new SimpleDialog(mMainFrame, "Discard edits to this file?", textArea);
        ...
        if (null != inputFile) {
            ...
            if (inputFile.exists()) {
                if (inputFile.isDirectory()) {
                    ...
                    SimpleDialog messageDialog = new SimpleDialog(mMainFrame, "Open cancelled", textArea);
                    messageDialog.show();
                } else {
        ...
        if (null != outputFile) {
            ...
            if (outputFile.exists()) {
                if (outputFile.isDirectory()) {
                    SimpleTextArea textArea = new SimpleTextArea("The file you selected is a directory:\n" + fileName + "\n");
                    SimpleDialog messageDialog = new SimpleDialog(mMainFrame, "Save cancelled", textArea);
                    messageDialog.show();
                } else {
                    if ((null != curFileName && fileName.equals(curFileName)) || null == curFileName) {
                        ...
                        SimpleDialog messageDialog = new SimpleDialog(mMainFrame, "Overwrite existing file?", textArea);
                        messageDialog.setMessageType(JOptionPane.QUESTION_MESSAGE);
                        messageDialog.setOptionType(JOptionPane.YES_NO_OPTION);
                        messageDialog.show();
                        Integer response = (Integer) messageDialog.getValue();
    ...
    private void initializeEditorTextArea(Container pPane) {
        ...
        JTextArea editorTextArea = new JTextArea(EDITOR_TEXT_AREA_NUM_ROWS, EDITOR_TEXT_AREA_NUM_COLS);
