...
public class SimulatorStochasticGibsonBruck extends SimulatorStochasticBase implements IAliasableClass, ISimulator {
    ...
    private static final double computeTimeToNextReaction(double[] pReactionProbabilities, int pIndex, double pTime, Random pRandomNumberGenerator) {
        double probRate = pReactionProbabilities[pIndex];
        double timeOfNextReaction = 0.0;
        if (0.0 < probRate) {
            timeOfNextReaction = pTime + chooseDeltaTimeToNextReaction(pRandomNumberGenerator, pReactionProbabilities[pIndex]);
        } else {
    ...
    private final void computePutativeTimeToNextReactions(Random pRandomNumberGenerator, double pStartTime, double[] pReactionProbabilities, Reaction[] pReactions) {
        ...
        for (int ctr = 0; ctr < numReactions; ++ctr) {
            double timeToNextReaction = computeTimeToNextReaction(pReactionProbabilities, ctr, pStartTime, pRandomNumberGenerator);
    ...
    private static final void updateReactionRateAndTime(SpeciesRateFactorEvaluator pSpeciesRateFactorEvaluator, SymbolEvaluatorChemSimulation pSymbolEvaluator, IndexedPriorityQueue pPutativeTimeToNextReactions, int pReactionIndex, Reaction[] pReactions, double[] pReactionProbabilities, Random pRandomNumberGenerator, int pLastReactionIndex) throws DataNotFoundException {
    ...
    protected final void prepareForStochasticSimulation(SpeciesRateFactorEvaluator pSpeciesRateFactorEvaluator, SymbolEvaluatorChemSimulation pSymbolEvaluator, double pStartTime, Random pRandomNumberGenerator, Reaction[] pReactions, double[] pReactionProbabilities) throws DataNotFoundException {
        computeReactionProbabilities(pSpeciesRateFactorEvaluator, pSymbolEvaluator, pReactionProbabilities, pReactions);

        computePutativeTimeToNextReactions(pRandomNumberGenerator, pStartTime, pReactionProbabilities, pReactions);
    }
    ...
    protected final double iterate(SpeciesRateFactorEvaluator pSpeciesRateFactorEvaluator, SymbolEvaluatorChemSimulation pSymbolEvaluator, double pEndTime, Reaction[] pReactions, double[] pReactionProbabilities, Random pRandomNumberGenerator, double[] pDynamicSymbolValues, MutableInteger pLastReactionIndex, DelayedReactionSolver[] pDelayedReactionSolvers) throws DataNotFoundException, IllegalStateException {
        ...
        if (NULL_REACTION != lastReactionIndex) {
            ...
            updateSymbolValuesForReaction(pSymbolEvaluator, lastReaction, pDynamicSymbolValues, pDelayedReactionSolvers);
            ...
            for (int ctr = numDependentReactions; --ctr >= 0;) {
                ...
                updateReactionRateAndTime(pSpeciesRateFactorEvaluator, pSymbolEvaluator, putativeTimeToNextReactions, dependentReactionCtr, pReactions, pReactionProbabilities, pRandomNumberGenerator, lastReactionIndex);
