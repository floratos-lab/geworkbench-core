...
import org.systemsbiology.util.AbstractComparator;
import org.systemsbiology.util.DataNotFoundException;
import org.systemsbiology.util.SlidingWindowTimeSeriesQueue;
...
public class DelayedReactionSolver extends Expression {
    ...
    private org.systemsbiology.util.PriorityQueue mReactionTimes;
    ...
    public DelayedReactionSolver(Species pReactant, Species pIntermedSpecies, double pDelay, double pRate, int pReactionIndex, boolean pIsStochasticSimulator) {
        ...
        if (mIsStochasticSimulator) {
            mReactionTimes = new org.systemsbiology.util.PriorityQueue(new AbstractComparator() {
    ...
    void initializeSpeciesSymbols(HashMap pSymbolMap, Species[] pDynamicSymbolValues, // a vector of all species in the model
                                  SymbolValue[] pNonDynamicSymbolValues) throws IllegalStateException {
        mIntermedSpecies = Reaction.getIndexedSpecies(mIntermedSpecies, pSymbolMap, pDynamicSymbolValues, pNonDynamicSymbolValues);

        mReactant = Reaction.getIndexedSpecies(mReactant, pSymbolMap, pDynamicSymbolValues, pNonDynamicSymbolValues);
    }
    ...
    public double computeValue(SymbolEvaluator pSymbolEvaluator) throws DataNotFoundException {
        ...
        if (!mIsStochasticSimulator) {
            ...
            if (intermedSpeciesValue >= 0.0 && peakTime >= minTime) {
                ...
                if (intermedSpeciesValue > 0.0 && intermedSpeciesValue > numIntermedSpeciesExpected) {
                    double rate = mRate;
    ...
    private static final double computeIntegral(SlidingWindowTimeSeriesQueue history, double h, int numTimePoints, double sqrtTwoPiNumStepsCorrected, double numStepsCorrected, double rate, double currentTime) {
        double value = 0.0;
        double prodRate = 0.0;
        int numPoints = history.getNumStoredPoints();

        for (int ctr = numPoints; --ctr >= 0;) {
            value = h * computeIntegrandValue(history, ctr, rate, rate * rate, sqrtTwoPiNumStepsCorrected, numStepsCorrected, currentTime);
    ...
    private static final double computeIntegrandValue(SlidingWindowTimeSeriesQueue pReactantHistory, int pTimePointIndex, double pRate, double pRateSquared, double pSqrtTwoPiNumStepsCorrected, double numStepsCorrected, double pCurrentTime) {
